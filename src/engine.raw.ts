...
export async function runRaw(url: string, opts: { headless?: boolean } = {}) {
  loadLearnedSelectors();
  ...
  const buckets: Record<string, string[]> = {
    // 1) proven winners (skip if weak profile)
    list: isCold ? [] : learnedList,
    // 2) specific “anchor” patterns
    anchors: unique([
      ...(learned.anchors || []),
      ...toArray(auto?.listSelector).map(toAnchorVariant),
      ...(auto?.candidates || []).filter(isAnchorish),
    ]),
    // 3) containers (cards/grids)
    containers: unique([
      ...(learned.containers || []),
      ...(auto?.candidates || []).filter(isContainerish),
    ]),
    // 4) general broad but useful
    broad: unique([
      ...(learned.broad || []),
      "a[href*='/product']",
      "a[href*='/products/']",
    ]),
    // 5) raw candidates (everything else)
    candidates: unique([
      ...(learned.candidates || []),
  ...
  /* progressive testing: try bucket-by-bucket */
  let items: any[] = [];
  ...
  for (const [bucketName, sels] of Object.entries(buckets)) {
    ...
    // single-pass tries (stop at first success)
    for (const sel of sels) {
      tried.push(sel);
      const res = extractItems(finalHtml, [sel], fields);
      if (res.length) {
        log.info(
          `[raw] ✓ success with ${bucketName}:`,
          sel,
          `→ ${res.length} items`
        );
        items = res;
  ...
  // 🧹 Normalize, dedupe, rescore
  items = postProcessItems(items, url);

  // ✅ Precision check (title + href present) AFTER post-processing
  ...
  log.info(
    ...
    (isCold || precision < 0.5) ? "new" : (profile?.id || "default"),
    ")"
  );

  const stats = getStats();
  log.info(
    `[raw] Summary → pages:${stats.pages} html:${stats.pagesHtml} items:${stats.items}`
  );
